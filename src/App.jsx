...
function build7DayData(events) {
  const days = Array.from({ length: 7 }, (_, i) => {
    ...
    return {
      ...
      falls: 0, gestures: 0, inactivity: 0,
    };
  });
...
function PatientCard({ patient, caregiver, lastEvent, onClick }) {
  ...
  return (
    <div
      ...
      style={{
        background: "#1e293b",
        border: `0.5px solid ${isAlert ? "#ef4444" : isWarn ? "#f59e0b" : "#334155"}`,
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
        <div style={{
          ...
          background: avatarBg(patient.id), color: "#00c2d4",
      ...
      <div style={{
        ...
        borderTop: "0.5px solid #334155", paddingTop: 8,
      }}>
...
function EventRow({ event, patientName }) {
  ...
  return (
    <div style={{
      ...
      padding: "9px 14px", borderBottom: "0.5px solid #1e293b50",
    }}>
      ...
      <div style={{ maxHeight, overflowY: "auto" }}>
        {filtered.length === 0 ? (
          ...
          filtered.slice(0, 100).map((e) => (
            <EventRow key={e.id} event={e} patientName={patientMap[e.patient_id] ?? e.device_id ?? "—"} />
          ))
        )}
      </div>
    </div>
  ...
  return (
    <div style={{ background: "#1e293b", border: "0.5px solid #334155", borderRadius: 10, padding: 14 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14, paddingBottom: 10, borderBottom: "0.5px solid #334155" }}>
        ...
        <button
          onClick={onBack}
          style={{ marginLeft: "auto", background: "#0f172a", border: "0.5px solid #334155", color: "#94a3b8", padding: "5px 10px", borderRadius: 6, cursor: "pointer", fontSize: 11 }}
        >
          ← Back
        </button>
      </div>
...
function WifiScans({ scans, patients }) {
  ...
  if (!scans.length) return (
    <div style={{ textAlign: "center", color: "#475569", padding: 40, fontSize: 12 }}>No WiFi scan data yet</div>
  );
  return (
    <div>
        ...
        <div style={{ maxHeight: 340, overflowY: "auto" }}>
          {Object.entries(grouped).map(([device, deviceScans]) => (
            <div key={device} style={{ padding: "10px 14px", borderBottom: "0.5px solid #334155" }}>
              <div style={{ fontSize: 11, color: "#00c2d4", fontWeight: 600, marginBottom: 8 }}>
                📡 {patientByDevice[device] ?? device}
              </div>
...
const CustomTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div style={{ background: "#0f172a", border: "0.5px solid #334155", borderRadius: 8, padding: "8px 12px", fontSize: 11 }}>
      ...
      {payload.map((p) => (
        <div key={p.dataKey} style={{ color: p.fill, marginTop: 2 }}>{p.name}: {p.value}</div>
      ))}
    </div>
  );
};
...
export default function App() {
  ...
  const todayFalls = events.filter((e) =>
    e.event_type === "fall_confirmed" &&
    new Date(e.created_at).toDateString() === new Date().toDateString()
  ).length;
  const alertCount = events.filter((e) =>
    ["fall_confirmed","gesture_help","inactivity"].includes(e.event_type)
  ).length;
  ...
  if (loading) return (
    <div style={{ ...S.root, display: "flex", alignItems: "center", justifyContent: "center", minHeight: 400, flexDirection: "column", gap: 14 }}>
      ...
      <span style={{ fontSize: 12, color: "#475569" }}>Connecting to Supabase...</span>
    </div>
  );
      ...
      <nav style={S.nav}>
        ...
        <div style={{ display: "flex", gap: 2, height: "100%", alignItems: "stretch" }}>
          {TABS.map((t) => (
            <div
              key={t.id}
              onClick={() => { setTab(t.id); setSelectedPatient(null); }}
              style={{
                padding: "0 14px", display: "flex", alignItems: "center",
                fontSize: 12, fontWeight: 500, cursor: "pointer",
                color: tab === t.id && !selectedPatient ? "#00c2d4" : "#64748b",
                borderBottom: `2px solid ${tab === t.id && !selectedPatient ? "#00c2d4" : "transparent"}`,
        ...
        <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 6, fontSize: 11, color: "#22c55e", fontWeight: 500 }}>
      ...
      <div style={S.body}>
        ...
        {detailPatient ? (
          <PatientDetail
            ...
            caregiver={cgMap[detailPatient.caregiver_id]}
            events={events}
          ...
          <>
            ...
            {/* Patient cards */}
            <div>
              <div style={{ fontSize: 11, fontWeight: 600, color: "#64748b", textTransform: "uppercase", letterSpacing: ".8px", marginBottom: 8 }}>
                Patients
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: 10 }}>
                {patients.map((p) => (
                  <PatientCard
                    key={p.id}
                    patient={p}
                    caregiver={cgMap[p.caregiver_id]}
                    lastEvent={patientLastEvent(p.id)}
